...
<div ng-controller="ctnCalculator as ctn">
	<div id="ctnConfig" style="width: 100%; margin-right: 0px; padding-left: 20px">
		...
		<p>Style: <select class="btn-success dropdown-toggle" ng-model="styleSelect" ng-options="x.style for x in ctnStyle" ng-init="styleSelect = ctnStyle[0]"> </select> <img ng-src="{{styleSelect.image}}" style="width: 10%; height: 10%"></p>
		...
		<p> Dimensions:
			...
			<div id="results" style="border: 1px solid grey; width: 20%; float: right; margin-top:-570px; padding-left: 5px; margin-right: 650px; border-radius: 10px">
				<h3>Results</h3>
				<p><strong>Board Size: </strong><span ng-style="blankWidth()" ng-if="calcBlankWidth() !==null" >{{calcBlankWidth()}}</span> x 
					<span ng-style="blankLength()" ng-if="calcBlankLength() !==null">{{calcBlankLength()}}</span></p>
